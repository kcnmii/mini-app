...
def upgrade() -> None:
    ...
    # ── 2. signatures table ──
    if "signatures" not in existing_tables:
        op.create_table(
            "signatures",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
    ...
    if "signing_sessions" not in existing_tables:
        op.create_table(
            "signing_sessions",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
    ...
    # ── 4. esf_records table ──
    if "esf_records" not in existing_tables:
        op.create_table(
            "esf_records",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
            sa.Column("document_id", sa.Integer(), sa.ForeignKey("documents.id", ondelete="CASCADE"), nullable=False, index=True),
    ...
    if "document_shares" not in existing_tables:
        op.create_table(
            "document_shares",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
            sa.Column("document_id", sa.Integer(), sa.ForeignKey("documents.id", ondelete="CASCADE"), nullable=False, index=True),
