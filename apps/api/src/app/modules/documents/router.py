...
async def list_recent_documents(db: Session = Depends(get_db)) -> list[DocumentRead]:
...
async def get_document(document_id: int, db: Session = Depends(get_db)):
    ...
    # Serialize object to dict since the app relied on dict access 
    doc_data = {
        "id": doc.id,
        "title": doc.title,
    ...
    if not doc_data.get("payload_json"):
        # Reconstruct basic payload from items table if json is missing
...
async def save_invoice_document(payload: SaveInvoiceRequest, db: Session = Depends(get_db)) -> DocumentRead:
